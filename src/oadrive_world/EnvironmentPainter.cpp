...
//    cv::Point2f center( realX*pixelsPerMeter, realY*pixelsPerMeter );
//    cv::RotatedRect rRect = cv::RotatedRect( center,
//                                             cv::Size2f(patch->getLength()*pixelsPerMeter, patch->getWidth()*pixelsPerMeter ),
//                                             patch->getPose().getYaw()*180.0/M_PI );
//
...
//    cv::Point2f center( realX*pixelsPerMeter, realY*pixelsPerMeter );
//    cv::RotatedRect rRect = cv::RotatedRect( center,
//                                             cv::Size2f(region->getLength()*pixelsPerMeter, region->getWidth()*pixelsPerMeter ),
                                  ...
                                  double radius, float pixelsPerMeter )
{
  ...
  double offsetDist = CAR_ORIGIN_TO_FRONT - env->mCar->getLength()*0.5;
...
//  }
  ...
  cv::RotatedRect rRect = cv::RotatedRect( center,
      cv::Size2f(env->mCar->getLength()*pixelsPerMeter, env->mCar->getWidth()*pixelsPerMeter ),
      env->mCar->getPose().getYaw()*180.0/M_PI );
  cv::Point2f vertices[4];
  rRect.points(vertices);
  for (int i = 0; i < 4; i++)

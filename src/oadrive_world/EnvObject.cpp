...
EnvObject::EnvObject(ExtendedPose2d pose, double width, double length)
{
  this->mPose = pose;
  this->mProbability = 1;
  this->mLikelyhood = 0;
...
void EnvObject::mergeFrom(const EnvObjectPtr obj)
{
  this->mPose = obj->mPose;
  this->mType = obj->mType;
  this->mlength = obj->mlength;
  this->mWidth = obj->mWidth;
  this->mTime = std::max(this->mTime, obj->mTime);
}

uint64_t EnvObject::getId() const {
  return mId;
}

void EnvObject::setId(uint64_t id) {
  mId = id;
}
...
double EnvObject::getLength() const
{
  return mlength;
}
...
double EnvObject::calcDistTo(EnvObjectPtr other) const
{
  return calcDistTo(other->getPose());
}

double EnvObject::calcDistTo(const ExtendedPose2d &pose) const
{
  return calcDistTo(pose.getPosition());
}

double EnvObject::calcDistTo(const Position2d &pos) const
{
  double distance =  std::sqrt((this->getX() - pos[0]) * (this->getX() - pos[0]) +
                    (this->getY() - pos[1]) * (this->getY() - pos[1]));
  return distance;
}

void EnvObject::calcMinDistanceToRefPoints(const ExtendedPose2d &pose)
{

  this->updateCorners();
  Position2d pos = pose.getPosition();
  double distA = std::sqrt((this->getCornerA()[0] - pos[0]) * (this->getCornerA()[0] - pos[0]) +
                           (this->getCornerA()[1] - pos[1]) * (this->getCornerA()[1] - pos[1]));
  double distB = std::sqrt((this->getCornerB()[0] - pos[0]) * (this->getCornerB()[0] - pos[0]) +
                           (this->getCornerB()[1] - pos[1]) * (this->getCornerB()[1] - pos[1]));
  double distC = std::sqrt((this->getCornerC()[0] - pos[0]) * (this->getCornerC()[0] - pos[0]) +
                           (this->getCornerC()[1] - pos[1]) * (this->getCornerC()[1] - pos[1]));
  double distD = std::sqrt((this->getCornerD()[0] - pos[0]) * (this->getCornerD()[0] - pos[0]) +
                           (this->getCornerD()[1] - pos[1]) * (this->getCornerD()[1] - pos[1]));
...
bool EnvObject::contains(double x, double y)
{
  if ((x >= this->getX() && x <= (this->getX() + this->getWidth())) &&
      (y >= this->getY() && y <= (this->getY() + this->getLength())))
  {
    return true;
  }
  else
...
bool EnvObject::isPointInside(const ExtendedPose2d &pose) const
{
  ...
  float AM_x = pose.getX() - mCornerA(0);
  float AM_y = pose.getY() - mCornerA(1);
...
void EnvObject::updateCorners()
{
  try
  {
    ...
    mCornerA = -0.5 * getLength() * unitX + 0.5 * getWidth() * unitY + getPose().getPosition();
    mCornerB = 0.5 * getLength() * unitX + 0.5 * getWidth() * unitY + getPose().getPosition();
    mCornerC = 0.5 * getLength() * unitX - 0.5 * getWidth() * unitY + getPose().getPosition();
    mCornerD = -0.5 * getLength() * unitX - 0.5 * getWidth() * unitY + getPose().getPosition();
...
unsigned int EnvObject::checkIntersections(const Position2d &start, const Position2d &end)
{
  ...
  Position2d mCornerA =
          -0.5 * getLength() * unitX + 0.5 * getWidth() * unitY + getPose().getPosition();
  Position2d mCornerB =
          0.5 * getLength() * unitX + 0.5 * getWidth() * unitY + getPose().getPosition();
...
bool EnvObject::checkOverlap(EnvObjectPtr other)
{
  ...
  if (this->isPointInside(otherCornerA) || this->isPointInside(otherCornerB) ||
      this->isPointInside(otherCornerC) || this->isPointInside(otherCornerD))
  {
    overlap = true;
  }
  if (other->isPointInside(thisCornerA) || other->isPointInside(thisCornerB) ||
      other->isPointInside(thisCornerC) || other->isPointInside(thisCornerD))
  {
    overlap = true;
  }
...
core::ExtendedPose2d EnvObject::getVelocity() const
{
  return mVelocity;
}

void EnvObject::setVelocity(const core::ExtendedPose2d &velocity)
{
  mVelocity = velocity;
}

        ...
        : mCoordConverter(converter)
{
  ...
  DebugViewRoadPerception::setHaarFilter(this);
}
...
void HaarFilter::startup()
{
  ...
  pixelsPerMeter = mCoordConverter->getPixelsPerMeterX();

  // initialize side line feature
  float haarFeatureSideStraightWidth = 0.035;
  float haarFeatureSideStraightHeight = 0.02;

  mHaarFeatureSideStraightY.initialize(haarFeatureSideStraightWidth, haarFeatureSideStraightHeight,
                                       pixelsPerMeter, 0.0);
  updateMaximumOffset(mHaarFeatureSideStraightY);
  mHaarFeatureSideStraightRotatedY.initialize(haarFeatureSideStraightWidth,
                                              haarFeatureSideStraightHeight, pixelsPerMeter,
                                              M_PI_4);
  updateMaximumOffset(mHaarFeatureSideStraightRotatedY);
  mHaarFeatureSideStraightX.initialize(haarFeatureSideStraightWidth, haarFeatureSideStraightHeight,
                                       pixelsPerMeter, M_PI_2);
  updateMaximumOffset(mHaarFeatureSideStraightX);
  mHaarFeatureSideStraightRotatedX.initialize(haarFeatureSideStraightWidth,
                                              haarFeatureSideStraightHeight, pixelsPerMeter,
                                              M_PI_2 + M_PI_4);
  updateMaximumOffset(mHaarFeatureSideStraightRotatedX);
  LOGGING_INFO(lanedetectionLogger, "[HaarFilter] side line features created with line width "
          << haarFeatureSideStraightWidth * pixelsPerMeter << "pixel" << endl);

  // initialize center line feature
  float haarFeatureCenterStraightWidth = 0.028;
  float haarFeatureCenterStraightHeight = 0.02;

  mHaarFeatureCenterStraightY.initialize(haarFeatureCenterStraightWidth,
                                         haarFeatureCenterStraightHeight, pixelsPerMeter, 0.0);
  updateMaximumOffset(mHaarFeatureCenterStraightY);
  mHaarFeatureCenterStraightRotatedY.initialize(haarFeatureCenterStraightWidth,
                                                haarFeatureCenterStraightHeight, pixelsPerMeter,
                                                M_PI_4);
  updateMaximumOffset(mHaarFeatureCenterStraightRotatedY);
  mHaarFeatureCenterStraightX.initialize(haarFeatureCenterStraightWidth,
                                         haarFeatureCenterStraightHeight, pixelsPerMeter, M_PI_2);
  updateMaximumOffset(mHaarFeatureCenterStraightX);
  mHaarFeatureCenterStraightRotatedX.initialize(haarFeatureCenterStraightWidth,
                                                haarFeatureCenterStraightHeight, pixelsPerMeter,
                                                M_PI_2 + M_PI_4);
  updateMaximumOffset(mHaarFeatureCenterStraightRotatedX);
  LOGGING_INFO(lanedetectionLogger, "[HaarFilter] center line features created with line width "
          << haarFeatureCenterStraightWidth * pixelsPerMeter << "pixel" << endl);

  // initialize crossroad line feature
  float haarFeatureCrossroadLineWidth = 0.045;
  float haarFeatureCrossroadLineHeight = 0.05;

  mHaarFeatureCrossroadLineY.initialize(haarFeatureCrossroadLineWidth,
                                        haarFeatureCrossroadLineHeight, pixelsPerMeter, 0.0);
  updateMaximumOffset(mHaarFeatureCrossroadLineY);
  mHaarFeatureCrossroadLineRotatedY.initialize(haarFeatureCrossroadLineWidth,
                                               haarFeatureCrossroadLineHeight, pixelsPerMeter,
                                               M_PI_4);
  updateMaximumOffset(mHaarFeatureCrossroadLineRotatedY);
  mHaarFeatureCrossroadLineX.initialize(haarFeatureCrossroadLineWidth,
                                        haarFeatureCrossroadLineHeight, pixelsPerMeter, M_PI_2);
  updateMaximumOffset(mHaarFeatureCrossroadLineX);
  mHaarFeatureCrossroadLineRotatedX.initialize(haarFeatureCrossroadLineWidth,
                                               haarFeatureCrossroadLineHeight, pixelsPerMeter,
                                               M_PI_2 + M_PI_4);
  updateMaximumOffset(mHaarFeatureCrossroadLineRotatedX);
  LOGGING_INFO(lanedetectionLogger,
               "[HaarFilter] cross road stop line features created with line width "
                       << haarFeatureCrossroadLineWidth * pixelsPerMeter << "pixel" << endl);

  // initialize crossroad line feature
  float haarFeatureCrossroadLineWidthWashedOut = 0.0009;
  float haarFeatureCrossroadLineHeightWashedOut = 0.05;

  mHaarFeatureCrossroadLineWashedOutRotatedY.initialize(haarFeatureCrossroadLineWidthWashedOut,
                                                        haarFeatureCrossroadLineHeightWashedOut,
                                                        pixelsPerMeter, M_PI_4);
  updateMaximumOffset(mHaarFeatureCrossroadLineWashedOutRotatedY);
  mHaarFeatureCrossroadLineWashedOutX.initialize(haarFeatureCrossroadLineWidthWashedOut,
                                                 haarFeatureCrossroadLineHeightWashedOut,
                                                 pixelsPerMeter, M_PI_2);
  updateMaximumOffset(mHaarFeatureCrossroadLineWashedOutX);
  mHaarFeatureCrossroadLineWashedOutRotatedX.initialize(haarFeatureCrossroadLineWidthWashedOut,
                                                        haarFeatureCrossroadLineHeightWashedOut,
                                                        pixelsPerMeter, M_PI_2 + M_PI_4);
  updateMaximumOffset(mHaarFeatureCrossroadLineWashedOutRotatedX);
  LOGGING_INFO(lanedetectionLogger,
               "[HaarFilter] washed out cross road stop line features created with line width "
                       << haarFeatureCrossroadLineWidthWashedOut * pixelsPerMeter << "pixel"
                       << endl);
  ...
  mHaarFeatureParkingLineX.initialize(haarFeatureParkingLineDeltaX, haarFeatureParkingLineDeltaY,
                                      pixelsPerMeter, M_PI_2);
  updateMaximumParkingOffset(mHaarFeatureParkingLineX);
  LOGGING_INFO(lanedetectionLogger,
               "[HaarFilter] parking line feature created with parking line width "
                       << haarFeatureParkingLineDeltaX * pixelsPerMeter << "pixel" << endl);

  // calculate factors between maximum responses to be able to compare responses (all scaled to one reference scale)
  factorSide2Center = (float) mHaarFeatureSideStraightY.getMaximumResponse() /
                      (float) mHaarFeatureCenterStraightY.getMaximumResponse();
  factorParkingLine2Center = (float) mHaarFeatureParkingLineX.getMaximumResponse() /
                             (float) mHaarFeatureCenterStraightY.getMaximumResponse();
  factorCrossroad2Center = (float) mHaarFeatureCrossroadLineY.getMaximumResponse() /
                           (float) mHaarFeatureCenterStraightY.getMaximumResponse();
  factorCrossroadWashedOut2Center =
          (float) mHaarFeatureCrossroadLineWashedOutX.getMaximumResponse() /
          (float) mHaarFeatureCenterStraightY.getMaximumResponse();
  factorCrossroad2Side = (float) mHaarFeatureCrossroadLineY.getMaximumResponse() /
                         (float) mHaarFeatureSideStraightY.getMaximumResponse();
  factorCrossroadWashedOut2Crossroad =
          (float) mHaarFeatureCrossroadLineWashedOutRotatedY.getMaximumResponse() /
          (float) mHaarFeatureCrossroadLineY.getMaximumResponse();
  ...
  else
  {
    ...
    LOGGING_ERROR(lanedetectionLogger,
                  "[HaarFilter] calculation of bird view cone unsuccessful, searching in whole image for features");
  }
...
void HaarFilter::setImage(cv::Mat &image)
{
  ...
  // add border to Image
  cv::copyMakeBorder(image, imageWithBorder, maxOffsetY, maxOffsetY, maxOffsetX, maxOffsetX,
                     cv::BORDER_REPLICATE);
...
void HaarFilter::calculateFeaturesPerLine(bool detectCrossroad /*= false*/,
                                          bool detectParkingLot /*= false*/)
{
  ...
  {
    LOGGING_ERROR(lanedetectionLogger,
                  "[HaarFilter] no valid image provided with setImage function");
  ...
  int highestResponsesSide[currentAmountResponsesSide][4];
  ...
  int highestResponsesCenter[currentAmountResponsesCenter][4];
  ...
  int highestResponsesCrossroad[AMOUNT_RESPONSES_CROSSROAD][4];
  if (detectCrossroad)
  {
  ...
  int highestResponsesParkingLine[currentAmountResponsesParkingLine][4];
  if (detectParkingLot)
  {
  ...
  for (int i = maxOffsetX, endI = maxOffsetX + mInputSize.width; i < endI; i += STEPSIZE_X)
  {
    ...
    for (int j = maxOffsetY; j < yMax; j += STEPSIZE_Y)
    {
      ...
      {
        saveHighestResponses(highestResponsesSide, currentAmountResponsesSide, weightSideY, x, y,
                             ORIENTATION_Y);
        saveHighestResponses(highestResponsesSide, currentAmountResponsesSide, weightSideRotatedY,
                             x, y, ORIENTATION_Y_ROTATED);
        saveHighestResponses(highestResponsesSide, currentAmountResponsesSide, weightSideRotatedX,
                             x, y, ORIENTATION_X_ROTATED);
      }
      ...
      {
        saveHighestResponses(highestResponsesCenter, currentAmountResponsesCenter, weightCenterY, x,
                             y, ORIENTATION_Y);
        saveHighestResponses(highestResponsesCenter, currentAmountResponsesCenter,
                             weightCenterRotatedY, x, y, ORIENTATION_Y_ROTATED);
        saveHighestResponses(highestResponsesCenter, currentAmountResponsesCenter,
                             weightCenterRotatedX, x, y, ORIENTATION_X_ROTATED);
      }
    }
  }
    ...
    for (int x = maxOffsetX, endX = mInputSize.width + maxOffsetX; x < endX; x += STEPSIZE_Y)
    {
      ...
      for (int y = maxOffsetY; y < yMax - 1; y += STEPSIZE_X)
      {
        ...
        if (y < DETECT_CROSSROAD_Y_FROM_TOP)
        {
          ...
          absoluteMaxSide = (float) max(weightSideX, max(weightSideRotatedY, weightSideRotatedX)) /
                            factorCrossroad2Center;
          ...
          absoluteMaxCrossroad = (float) max(weightCrossroadX, max(weightCrossroadRotatedY,
                                                                   weightCrossroadRotatedX)) /
                                 factorCrossroadWashedOut2Center;
        ...
        if (absoluteMax == absoluteMaxSide)
        {
          if (detectParkingLot)
          {
            saveHighestResponses(highestResponsesParkingLine, currentAmountResponsesParkingLine,
                                 weightSideX, x, y, ORIENTATION_X);
          }
          else
          {
            saveHighestResponses(highestResponsesSide, currentAmountResponsesSide, weightSideX, x,
                                 y, ORIENTATION_X);
          }
          saveHighestResponses(highestResponsesSide, currentAmountResponsesSide, weightSideRotatedY,
                               x, y, ORIENTATION_Y_ROTATED);
        ...
        {
          saveHighestResponses(highestResponsesCrossroad, AMOUNT_RESPONSES_CROSSROAD,
                               weightCrossroadX, x, y, ORIENTATION_X);
          saveHighestResponses(highestResponsesCrossroad, AMOUNT_RESPONSES_CROSSROAD,
                               weightCrossroadRotatedY, x, y, ORIENTATION_Y_ROTATED);
          saveHighestResponses(highestResponsesCrossroad, AMOUNT_RESPONSES_CROSSROAD,
                               weightCrossroadRotatedX, x, y, ORIENTATION_X_ROTATED);
        }
      }
    }
  }
...
std::cout << "max side (vertical): " << totalMaxSide << std::endl;*/
  ...
  if (detectParkingLot)
  {
    ...
    for (int x = max(offsetX, maxOffsetX), endX =
            mInputSize.width + maxOffsetX - (offsetParkingX - maxOffsetX);
         x < endX; x += STEPSIZE_PARKING_X)
    {
      y1 = mLeft * (x - maxOffsetX) + bLeft;
      y2 = mRight * (x - maxOffsetX) + bRight;
      ...
      for (int y = max(offsetY, maxOffsetY); y < yMax; y += STEPSIZE_PARKING_Y)
      {
        ...
        if (counterParkingLine == 0)
        {
          ...
          saveHighestResponses(highestResponsesParkingLine, currentAmountResponsesParkingLine,
                               weightParkingLineX, x, y, ORIENTATION_X);
        }
  ...
  updateMovingAverage(responsesSideStraight, LEFT_SIDE_LINE);
  updateMovingAverage(responsesCenterStraight, CENTER_LINE);
  if (detectCrossroad)
  {
    updateMovingAverage(responsesCrossroadStraight, CROSSROAD_LINE);
    overallMovingAverage = max(movingAverages[CENTER_LINE],
                               ((int) ((float) movingAverages[CROSSROAD_LINE] /
                                       factorCrossroad2Center)));
  }
  else
  {
  ...
  overallMovingAverage = max(((int) ((float) movingAverages[LEFT_SIDE_LINE] / factorSide2Center)),
                             overallMovingAverage);

  // update minimum response for every feature type
  movingAverages[LEFT_SIDE_LINE] = overallMovingAverage * factorSide2Center;
  ...
  extractStraightFeatures(highestResponsesSide, gridResponses, currentAmountResponsesSide,
                          &responsesSideStraight, LEFT_SIDE_LINE, &mHaarFeatureSideStraightY,
                          &mHaarFeatureSideStraightX, &mHaarFeatureSideStraightRotatedY,
                          &mHaarFeatureSideStraightRotatedX);
  // put center straight features into grid
  extractStraightFeatures(highestResponsesCenter, gridResponses, currentAmountResponsesCenter,
                          &responsesCenterStraight, CENTER_LINE, &mHaarFeatureCenterStraightY,
                          &mHaarFeatureCenterStraightX, &mHaarFeatureCenterStraightRotatedY,
                          &mHaarFeatureCenterStraightRotatedX);
  ...
  if (detectCrossroad)
  {
    ...
    extractStraightFeatures(highestResponsesCrossroad, gridResponses, AMOUNT_RESPONSES_CROSSROAD,
                            &responsesCrossroadStraight, CROSSROAD_LINE,
                            &mHaarFeatureCrossroadLineY, &mHaarFeatureCrossroadLineX,
                            &mHaarFeatureCrossroadLineRotatedY, &mHaarFeatureCrossroadLineRotatedX);
  }
  ...
  if (detectParkingLot)
  {
    ...
    extractStraightFeatures(highestResponsesParkingLine, NULL, currentAmountResponsesParkingLine,
                            NULL, PARKING_LINE, &mHaarFeatureParkingLineX, NULL, NULL, NULL);
  }
  ...
  for (int i = 0; i < gridCells; i++)
  {
    ...
    {
      Feature newFeature;
      newFeature.localPose = mCoordConverter->pixel2Car2(cv::Point2f(gridResponses[i][2] -
                                                                     maxOffsetX,
                                                                     gridResponses[i][3] -
                                                                     maxOffsetY));
      newFeature.localPose.setYaw(gridResponses[i][4]);;

      if (newFeature.localPose.getYaw() > M_PI_2)
      {
        newFeature.localPose.setYaw(newFeature.localPose.getYaw() - M_PI);
      }
...
{
  int movingAverage = 0;
  for (std::list<int>::const_iterator iterator = lastResponses.begin(), end = lastResponses.end();
...
void HaarFilter::extractStraightFeatures(int highestResponses[][4], float** gridResponses,
                                         int amountResponses, list<int>* lastResponses,
                                         ...
                                         HaarFeature* mHaarFeatureStraightRotatedX)
{
  ...
  for (int i = 0; i < amountResponses; i++)
  {
    if (highestResponses[i][FILTER_RESPONSE] > movingAverages[type])
    {
      ...
      probability = min((float) 1.0,
                        (float) highestResponses[i][FILTER_RESPONSE] / maximumResponse);
      if (type == PARKING_LOT)
      {
        angle = ANGLE2;
      }
      ...
      {
        angle = getAngle((Orientations) highestResponses[i][FILTER_ORIENTATION],
                         highestResponses[i][FILTER_RESPONSE], highestResponses[i][POSITION_X],
                         highestResponses[i][POSITION_Y], mHaarFeatureStraightY,
                         mHaarFeatureStraightX, mHaarFeatureStraightRotatedY,
                         mHaarFeatureStraightRotatedX);
      }
      ...
      else
      {
        ...
        newFeature.localPose = mCoordConverter->pixel2Car2(cv::Point2f(
                highestResponses[i][POSITION_X] - maxOffsetX,
                highestResponses[i][POSITION_Y] - maxOffsetY));
        if (angle > M_PI_2)
        {
          angle -= M_PI;
        }

        newFeature.localPose.setYaw(angle);
...
float HaarFilter::getAngle(Orientations orientation, int response, int positionX, int positionY,
                           HaarFeature* mHaarFeatureStraightY, HaarFeature* mHaarFeatureStraightX,
                           HaarFeature* mHaarFeatureStraightRotatedY,
                           HaarFeature* mHaarFeatureStraightRotatedX)
{
  float angle = 0.0;
...
void HaarFilter::saveHighestResponses(int highestResponses[][4], int amountResponses, int response,
...
{
  FeaturePointerVector featurePointer(mFeatureVector.size());
  for (int i = 0; i < mFeatureVector.size(); i++)
  {
    featurePointer.at(i) = &mFeatureVector.at(i);
  }

  return DebugViewRoadPerception::generateFeatureDebugImage(featurePointer);
}

float
HaarFilter::interpolateAngle(int weight, float angle, int weightNeighbour1, float angleNeighbour1,
                             int weightNeighbour2, float angleNeighbour2)
{
  int factor = 3;
  ...
  return (weight * angle + weightNeighbour1 * angleNeighbour1 +
          weightNeighbour2 * angleNeighbour2) / (weight + weightNeighbour1 + weightNeighbour2);
}
